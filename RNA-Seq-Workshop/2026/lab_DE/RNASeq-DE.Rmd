...
#### DESeq2 installation
```{r, eval = T, error=F, message=F, warning=F, results='hide'}
...
if (!require("ggplot2", quietly=T)) {
  biocmanger_version <- BiocManager::version()
  if (biocmanger_version == "3.18") {
...
```{r, echo=F, comment="", message = F, error = F}
if (!require("kableExtra", quietly=T))
...
```{r, comment="", message=F, warning=F, results="hide"}
library(GO.db)
library(ggplot2)
...
```{r, comment=""}
# GO DB (2 columns)
go <- read.delim("https://raw.githubusercontent.com/liu3zhenlab/teaching/refs/heads/master/RNA-Seq-Workshop/2026/lab_DE/data/go.txt", header=F)
...
dn_gene <- out$Gene[dn]
```
In total, `r length(up_gene)` and `r length(dn_gene)` up- and down-regulated genes were identified, respectively.


---
# GO enrichment test
```{r, fig.height = 5.5, dev='png', fig.align='center', message=F, warning=F}
